...
by a constant, which is where both $O(1/n)$ rates come from. A stability lemma for the pairing carries proofs across the relation in both directions: the coverage floor of \citet{barberpananjady2026} for split conformal on $\beta$-mixing
series factors into the lemma's two Lipschitz legs (buffer deletion as a measure perturbation; the switch coefficient as a distributional total variation), which is why their rate is linear
...
A relation of this kind is worth stating only if arguments cross it. Sections~\ref{sec:conformal}
...
constructs the measure, $\nu_n=\frac1n\sum_t\delta_{x_t}$. Conformal calibration fixes the measure, the empirical distribution of scores, and constructs the test $h_{q_n}$; its
...
$s\mapsto\chi_i(s)$ over placements $i$ of the evaluation index (equivalently, over the orbit $\{\rho^{*}(\pi)\,e^{*}_{n+1}\}$ of the evaluation functional at fixed data), and
...
proofs, not priority of authors.
\end{remark}
...
and pays the Monte Carlo rate $\sqrt{\tau/n}$. The linearity of the floor is the linearity of the pairing.
...
the guarantee is the deterministic one above, against the marginal law. The trade, symmetry for determinism, is the design of a measure-side controller for an index-side guarantee.
...
remains a property of the scores \citep{cotton_marginally}. Whether the steering can be aimed conditionally is open: selecting or reweighting calibration slots to balance the rank count given the input, as herding aims its particles at a target. Localized and weighted conformal

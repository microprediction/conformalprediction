...
smoothness are not. A plug-in estimate can be narrower, because it declines to insure
against alternatives the data cannot rule out, though whether narrower is better depends on
...
converge to an oracle band at the minimax rate, and \citet{guan2023localized} weights
calibration points by proximity to the test point while retaining exact finite-sample marginal
validity, by adjusting the quantile level rather than using the nominal one. The cost we identify attaches not to
...
The two halves of Proposition~\ref{prop:attain} are conditioned differently, and the
statement should be read with that in mind. Honesty holds conditional on the design
$X_1,\dots,X_n$, for every $\theta$, exactly and at every $n$: the proof fixes the design and
the only randomness left is the Gaussian $Z$. The sharpness bound does not hold conditionally.
It averages over the random design, through $\E\rho_k^{s}\le C'(k/n g(x_0))^{s/d}$ under
Assumption~\ref{as:design}, and a design whose $k$th nearest neighbour is unusually distant
carries a correspondingly larger excess. So the guarantee is design-conditional and the rate
is a design-averaged statement. Nothing here claims a design-conditional rate.

Three further points. The construction is a genuine finite-sample, design-conditional honesty
...
Theorem~\ref{thm:adapt} is a specialization, not a new principle. The honesty--adaptation
obstruction is due to \citet{low1997nonparametric} and \citet{cai2004adaptation}, whose
adaptation theory for nonparametric confidence intervals supplies the machinery the proof
uses. What is done here is to instantiate it for a conditional quantile threshold, with the
prediction-set notion of honesty in Definition~\ref{def:honest}. The same remark applies to
Theorem~\ref{thm:main}, whose engine is Le~Cam two-point testing. The contribution of this
note is not a new Le~Cam principle. It is the precise honesty--sharpness boundary for
prediction sets, and in particular the separation of three requirements that the conformal
literature does not usually distinguish: marginal conformal validity, fixed-$x_0$ honesty,
and confidence over the calibration sample.

A sharp point estimator can read apparent
...
thesis: where the local estimator is narrower, it is narrower not by estimating better but by
declining to insure against what it cannot see. Whether narrower is better depends on the
score being optimized, and is not settled here.
...
that reweight by proximity are marginally valid and can be sharp, provided the quantile level
is adjusted for the weighting \citep{guan2023localized}. The obstruction is
...
\bibitem[Cai and Low(2004)]{cai2004adaptation} Cai, T.~T. and Low, M.~G. (2004). An
  ...
  32(5), 1805--1840.
\bibitem[Guan(2023)]{guan2023localized} Guan, L. (2023). Localized conformal prediction: a
  generalized inference framework for conformal prediction. \emph{Biometrika} 110(1), 33--50.

...
smoothness are not. A plug-in estimate is sharper because it declines to insure against
...
\citep{li1989honest,low1997nonparametric,cai2004adaptation}. Here \emph{local} means the
requirement is imposed at the fixed point $x_0$ rather than over an interval.}
\end{definition}
...
All the proof uses is the small-ball relation $\Prob_X\{X\in B(x_0,h)\} = \lambda(x_0)\,
h^{d}(1+o(1))$, of which the density condition is the Euclidean special case with
$\lambda(x_0)=g(x_0)$. The small-ball form also covers smooth manifolds, non-Euclidean
metrics, and an intrinsic dimension $d$ below the raw feature dimension. Without the
Lebesgue point the local mass is only pinned between $g_-h^d$ and $g_+h^d$, and every rate
below holds with $g(x_0)$ replaced by a constant in $[g_-,g_+]$. The assumption fails at
boundaries and in sparse high-dimensional regions, where the local sample size is smaller
still and the inflation only grows.
...
the submodel applies to every larger class containing it. A sufficient condition is
differentiability in quadratic mean with bounded Fisher information and $q_\alpha'(u)$
bounded above and away from zero, whence the mean-value theorem gives the quantile
inequalities. The two-sided quantile bound forces $q_\alpha$ strictly monotone, so we
...
Write $s=k+\gamma$ with $k=\lfloor s\rfloor$ and $0<\gamma\le1$, and let $\Hs(L)$ be the
ball of fields $\theta$ with $k$th derivatives $\gamma$-H\"older with seminorm $\le L$ and
range $\theta(x)\in[\underline u,\overline u]$. Fix a bump profile $\psi$ with $\psi(0)=1$,
...
broad H\"older classes that permit localized features at the detection boundary, but it can
fail under global parametric structure, analyticity, monotonicity, or self-similarity, and
those are exactly the restrictions that can restore honest adaptation. Within a H\"older
...
$n_0$, depending only on $(\alpha,\eta,K,c_q,g_+,\psi,a_0,h_0)$ and the distance from $u_0$
to the parameter boundary, and a fixed constant baseline $\theta_0\in\Hs(L)$, such that for
all $n\ge n_0$ every $T_n$ satisfying
...
An honest procedure attains the lower-bound rate, so Theorem~\ref{thm:main} is a real
optimum and not an empty constraint. We show this in the lognormal model (B), where the
finite-sample honesty statement is exact rather than an $O_P$ assertion. Assume
$0<s\le1$, $0<\eta<\tfrac12$, and
\[
...
\eqref{eq:honest} conditional on the design, and with
$k \asymp L^{-2d/(2s+d)}\big(n g(x_0)\big)^{2s/(2s+d)}$ its excess is $O(r_n)$.
\end{proposition}

\begin{proof}
Conditional on $X_1,\dots,X_n$, $\widehat\theta_k = k^{-1}\sum_{i\in N_k}\theta(X_i) +
N(0,1/k)$, and H\"older smoothness with $s\le1$ gives $|\theta(x_0)-k^{-1}\sum_{i\in
N_k}\theta(X_i)| \le L\rho_k^{s}$. The oracle threshold is
$q_\alpha(\theta(x_0))=\exp\{\theta(x_0)+z_{1-\alpha}\}$, so $T_{n,k}(x_0)\ge
q_\alpha(\theta(x_0))$ iff the standard normal $Z=\sqrt k(\widehat\theta_k-k^{-1}\sum\theta(X_i))$
satisfies $Z \ge \sqrt k\,(\theta(x_0)-k^{-1}\sum\theta(X_i)-L\rho_k^{s}) - z_{1-\eta}$. The
right side is $\le -z_{1-\eta}$, and $\Prob\{Z\ge -z_{1-\eta}\}=1-\eta$, giving conditional
coverage at least $1-\eta$ for every $\theta$. Under Assumption~\ref{as:design},
$\rho_k^{s}\asymp (k/n g(x_0))^{s/d}$, so the excess is of order $L(k/n g(x_0))^{s/d} +
k^{-1/2}$, and the stated $k$ balances the two terms at
$L^{d/(2s+d)}(n g(x_0))^{-s/(2s+d)}=r_n$.
\end{proof}
...
average has bias $O(L\rho_k^{s})$. For $s>1$ the local average is replaced by a
local-polynomial estimate of degree $\lfloor s\rfloor$, which restores the bias order.
...
Cam boundary. Below the critical exponent the bump is detectable and $\TV\to1$; above it,
...
the uncorrected plug-in threshold: the honest band holds $1-\eta$ coverage at the baseline
while the plug-in covers about half the time, at the cost of the $r_n$ inflation the theorem
predicts. A random-design exponential experiment reproduces every scaling by Monte Carlo.
...
\caption{Honest $k$-nearest-neighbour band \eqref{eq:knn} versus the uncorrected plug-in
threshold, lognormal scores at the smooth baseline. The honest band holds coverage at
$1-\eta=0.9$; the plug-in covers about half the time. The gap is the $r_n$ inflation the
lower bound forces, and it decays at the predicted $n^{-1/3}$.}
\label{fig:attain}
\end{figure}
...
$\eta>0$}, and Section~\ref{sec:attain} attains it; at $\eta=0$ even that fails

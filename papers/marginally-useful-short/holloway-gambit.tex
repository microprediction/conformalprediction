...
\hypersetup{colorlinks=true,linkcolor=black,citecolor=black,urlcolor=blue,
  pdftitle={The Holloway Gambit},pdfauthor={Peter Cotton}}
...
\title{The Holloway Gambit:\\ The Willing Sacrifice in Split Conformal Prediction}

\author{Peter Cotton\thanks{Global Strategic Minerals Corporation.
  Email: \texttt{peter.cotton@gsmc.ai}. Code and figures are at
  \url{https://github.com/microprediction/conformalprediction}. A longer treatment,
  with the omitted readings of the identity and the time-series discussion, is
  \citet{cotton2026marginally}.}}
\date{}
...
\section{Coverage does not constrain the score}
...
How could it be otherwise? The spread never entered \eqref{eq:q}.

\section{The gap}

A conformal predictor can be made to output a distribution rather than a set, the
\emph{conformal predictive system} (CPS) of \citet{vovk2019nonparametric}, built from
\emph{signed} residual scores. We view it as an attempt to fix the residuals subject to the
restriction of using only one unconditional shared shape, as compared to a data-dependent
scale such as one would see in a GARCH model. It is the mode in which conformal prediction
estimates a full distribution, so it is the form the criticism must address.

Fix a location predictor $\hat\mu$ with residual $R=Y-\hat\mu(X)$ of marginal density $g$
...
nothing else. Write the true conditional residual density as $r(\cdot\given x)$, so the
oracle density is $q^\star(y\given x)=r(y-\hat\mu(x)\given x)$, and let $\bar r=g$ be the
...
The theorem is a small one. Its interest is in what it prices. The right-hand side of
...
gives ``blind to conditional residual shape'' a number. Note what the result does not say.
...
costs an additional $\KL(\bar r\,\|\,h_{\mathrm{sym}})$ for discarding sign information, at
most one bit; the structural loss is $I(R;X)$.

\paragraph{A false-pooling cost.} A single-shape system asserts that, once $\hat\mu$ is
fixed, one residual law fits everyone. That is the assertion $R\perp X$. The gap is the
...
$\E_X\,\KL(r(\cdot\given X)\,\|\,\bar r)=I(R;X)$. The conformal step fixes the level of the
book, meaning its normalization and hence its coverage. It does not touch the rent.
...
\section{What follows in practice}

Coverage error and proper-score performance are two coordinates, not one. Residual split
...
Three suggestions follow, and none is exotic. Conformalize \emph{last}, since the conformal
step cannot improve any proper score, so one should estimate first and certify second. Use a
proper score as the sharpness diagnostic, and note that because conformalization leaves it
unchanged, a score that does not move after conformalizing is confirmation that whatever
gain one has came from the modeling. And to close $I(R;X)$, add conditioning to the model.
Do not add tightness to the conformal step. There is nothing there to tighten.
...
this note we go beyond the usual caution (against reading the guarantee as forecast quality)
and we have emphasized the permanent structural concession made at the point residual
pooling is chosen. Our message is only that nothing comes without a price. The unconditional
coverage guarantee is accompanied by an equally rigorous guarantee that the conformal
information gap will never be crossed.
...
the truth in this region?}, and in forecasting that is frequently not the question anyone was
asking. This is not weak uncertainty quantification. It is exact uncertainty quantification
for a set, a coverage guarantee, no more and no less. The gambit is permanent. Its price is
$I(R;X)$, and the modeler should know what has been paid.

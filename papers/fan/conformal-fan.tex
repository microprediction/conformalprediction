...
is positively associated, not NA. The one hypothesis a reader might tighten is full support: the density vanishes wherever
two coordinates are large, so the question reopens if one restricts to strictly-positive-density NA laws
or to a fixed copula family --- which is where the positive results below live.

So the fan does not contract at every level for \emph{arbitrary} negative association. It does contract
on the structured families a calibration design actually produces, and there the statement is exact or
provable.

...
convex-order contraction of the exceedance count, supported numerically across dependence
structures. The governing sign is the
...
and to leading order everywhere (Theorem~\ref{thm:coeff}). What is left open is the exact
finite-sample inequality for an \emph{arbitrary} negatively associated law.
...
independent value \eqref{eq:betavar}.
\end{conjecture}

One ingredient is already exact. The exceedance indicators are negatively associated with
$\mathrm{Bernoulli}(u)$ marginals, so by \citet{shao2000} the count is convex-order dominated by the
independent binomial, $N(u)\le_{\mathrm{cx}}\mathrm{Bin}(n,u)$, with the same mean $nu$: a
mean-preserving contraction at every level. Carrying this to $\operatorname{Var}(c)$ needs the level
comparison $\mathbb P(N(u)\ge k)$ to cross the binomial tail once in $u$, which the total positivity
of the binomial tail in $(k,u)$ \citep{karlin1968} makes plausible, followed by a
...
below satisfy. We verify the conjecture across them (one run each, reproduced by

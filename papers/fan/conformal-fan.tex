...
extreme order statistic is strictly over-dispersed, at every $n$. The bound does hold on the
structured subclasses a calibration design produces --- Gaussian-copula, FGM, and
without-replacement --- which we prove.
\end{abstract}
...
operationally relevant one, is not --- and, it turns out, cannot be: the per-level bound is
...
is exactly the centroid-violating diagonal the linear program of \eqref{eq:centroid} produces: the hope
there --- that no genuine NA law realises it --- is false at every $n$, not merely at $n=2$. The
...
One hypothesis the counterexample uses is degeneracy: its density vanishes wherever two coordinates are
large, and it sits exactly on the boundary of negative association (its defining covariances are zero,
not strictly negative). Every full-support NA family we can analyse --- Gaussian copula, FGM --- instead
\emph{satisfies} the level bound. This suggests the degeneracy is essential, and leaves the natural
restricted question open.

\begin{conjecture}[Full-support per-level contraction]\label{conj:fullsupp}
If the calibration scores have a negatively associated law with a strictly positive continuous density
on $[0,1]^n$ (and uniform marginals), then $\operatorname{Var}(c)\le$ the independent value
\eqref{eq:betavar} for each $k$.
\end{conjecture}

So the fan does not contract at every level for \emph{arbitrary} negative association --- but the
violators are boundary, vanishing-density laws, and on the structured families a calibration design
actually produces the statement is exact or provable.
...
Sampling without replacement from a finite population is the second: there
$\operatorname{Var}(U_{(k)})$ is again a functional of the single tail curve $\mathbb P(N(u)\ge k)$, the
hypergeometric-versus-binomial difference of tails crosses once (Karlin's total positivity), and the
resulting centroid inequality gives the bound at every $k$ (proved outright at the extremes and for the
equal-mean central level, and verified for all populations of size $\le 60$). And in the canonical
continuous family the per-level statement is exact, and more, monotone in the correlation.

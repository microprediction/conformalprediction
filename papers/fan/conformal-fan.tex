...
reshapes it. The organizing identity is that $c$ is a functional of the exceedance-count process
...
so the average pairwise covariance of the exceedance indicators sets the leading constant. An exact
...
$1/n$. The question of this note is what the dependence among the calibration scores does to it.
...
statement is about the across-dataset fluctuation of $c$.
...
Let $p=1-\alpha$ and $\xi_i=\mathbf 1\{U_i\le p\}$, so $N(p)=\sum_i\xi_i$.

\begin{theorem}[Fan coefficient]\label{thm:coeff}
Suppose $k/n\to p\in(0,1)$ and the scores satisfy a Bahadur representation
$U_{(k)}-p=p-N(p)/n+r_n$ with the remainder negligible in $L^2$ relative to $n^{-1}
\operatorname{Var}(N(p))^{1/2}$. Then
\begin{equation}
...
$np(1-p)+n(n-1)\bar c$ with $\bar c=\tfrac1{n(n-1)}\sum_{i\ne j}\operatorname{Cov}(\xi_i,\xi_j)$,
\begin{equation}
\operatorname{Var}(c)=\frac{p(1-p)}{n}\,R\,(1+o(1)),\qquad R=1+\frac{n-1}{p(1-p)}\,\bar c .
\end{equation}
\end{theorem}

So the leading fan coefficient is the exceedance-count variance at the operating quantile. Under
...
is the between-$Q$ term, the signature of positive dependence: a per-dataset latent shifts the whole
...
Independent scores give $c\sim\mathrm{Beta}(k,n-k+1)$, the benchmark fan. At the maximally negatively
associated floor, take the discrete rank idealization in which the calibration scores are a uniformly
random permutation of the grid $\{1/(n+1),\dots,n/(n+1)\}$ (sampling without replacement, pairwise
correlation $-1/(n-1)$): the order statistic is deterministic, $U_{(k)}=k/(n+1)$ a.s., so
...
$\tfrac{n-1}n$ (the diagonal of $I-\tfrac1nJ$), giving constant at least $\tfrac n{n-1}$.
\end{proof}
...
$\tfrac{n-1}na^2+b^2=1$, this reduces to $m'(q)\le (n-1)a/n$, where $m(q)=\mathbb E[D\mid Y=q]$.
...
pre-specified, exchangeability-preserving randomized design (or covariate stratification independent
of the response) can lower the run-to-run variance of the threshold without disturbing validity. Exact
zero fan is the discrete rank/grid idealization; continuous-marginal designs (Latin hypercube) give a
small but nonzero fan, of order $1/n^2$.
\end{remark}
...
contraction is left as a conjecture.

...
associated sequences under mild conditions, gives $U_{(k)}-p = p-N(p)/n + o_P(n^{-1/2})$. Because
$U_{(k)}$ and $N(p)/n$ both lie in $[0,1]$, the remainder is bounded, so the convergence is also in
$L^2$ and the variance passes to the limit; hence

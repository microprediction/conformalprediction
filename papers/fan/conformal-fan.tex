...
$\operatorname{Var}(Z_{(k)})=v_k+\rho(1-v_k)$. The general single-level contraction turns out to
fail at $n=2$: we exhibit an explicit negatively associated pair, uniform marginals, whose
extreme order statistic is strictly over-dispersed. For $n\ge3$ it is open and unbroken across an
extensive search, and holds on the Gaussian-copula, FGM, and without-replacement subclasses.
\end{abstract}
...
\section{The single level: a counterexample at $n=2$, and a conjecture for $n\ge3$}

Everything above is proved for general negative association. The single conformal level, the operationally relevant one, is not, and the obstruction is exactly why the aggregate succeeds: the
...
and it does. That gap is not merely technical. At $n=2$ the single-level bound is \emph{false}.

\begin{proposition}[Counterexample at $n=2$]\label{prop:counter}
For each $a\in\bigl(0,\tfrac{\sqrt{11}-3}{2}\bigr)$ there is a negatively associated pair with uniform
marginals whose extreme order statistic is strictly over-dispersed:
$\operatorname{Var}(U_{(2)})=\tfrac1{18}+\tfrac{a^2(1-6a-2a^2)}{18}>\tfrac1{18}
=\operatorname{Var}^{\mathrm{iid}}(U_{(2)})$.
\end{proposition}

\begin{proof}
Split $[0,1]$ into a bottom bin $B=[0,1-a]$ and a top bin $T=[1-a,1]$. Draw the cell of $(U_1,U_2)$
with $\mathbb P(B,T)=\mathbb P(T,B)=a$, $\mathbb P(B,B)=1-2a$, $\mathbb P(T,T)=0$, and, given the cell,
make the coordinates independent uniforms on their bins: the two points are \emph{never both large}.
Each marginal is uniform (mass $a$ spread uniformly on $T$, mass $1-a$ on $B$). For a pair, negative
association is equivalent to negative quadrant dependence $C(u,v)\le uv$ \citep{joagdev1983,lehmann1966};
here $uv-C(u,v)=a^2\,(g_B(u)-g_T(u))(g_B(v)-g_T(v))\ge0$ with $g_B(x)=\min(x/(1-a),1)$,
$g_T(x)=\max(0,(x-(1-a))/a)$ and $g_B\ge g_T$ pointwise, so the law is genuinely NA. Its diagonal is
$\delta(u)=\mathbb P(U_{(2)}\le u)=(1-2a)u^2/(1-a)^2$ on $[0,1-a]$ and $2u-1$ on $[1-a,1]$; with
$\mathbb E[U_{(2)}^m]=1-m\int_0^1 u^{m-1}\delta(u)\,du$ this gives the stated variance, positive on the
displayed range of $a$.
\end{proof}

The excess is small but exact: at $a=\tfrac1{10}$, $\operatorname{Var}(U_{(2)})=\tfrac{1673}{30000}$, a
ratio $1.0038$ to the independent fan; optimising the diagonal (a Bertino copula) reaches about
$1.034$. By the negation symmetry $U\mapsto 1-U$, which preserves NA and uniform marginals, the mirror
law (never both \emph{small}) over-disperses the minimum by the same amount, so the bound fails at both
extreme levels. This is exactly the diagonal the linear program of \eqref{eq:centroid} below produces:
the note's hope there --- that no genuine NA law realises a centroid-violating diagonal --- is false at
$n=2$. The mechanism is that uniform marginals and exchangeability pin only the \emph{sum} of the
order-statistic means (to $n/2$), never an individual $\mathbb E[U_{(k)}]$, so a mean-decreasing spread
is available: the counterexample lowers $\mathbb E[U_{(2)}]$ while widening it, and the aggregate bound
survives because the other level over-contracts to compensate
($\operatorname{Var}(U_{(1)})+\operatorname{Var}(U_{(2)})\approx0.104<\tfrac2{18}$).

\begin{remark}[Why $n=2$ is special]
At $n=2$ the order-statistic variance is a functional of the copula diagonal $\delta(u)=C(u,u)$
\emph{alone}, and negative association collapses to the single diagonal constraint $\delta(u)\le u^2$
--- for a pair, NA, NQD, and negative correlation of monotone functions all coincide. That one-dimensional
constraint leaves room to push the deficit $u^2-\delta$ toward $u=1$, which is precisely the
over-dispersion. For $n\ge3$ the single-level variance depends on the off-diagonal negative-orthant
probabilities across thresholds (the content \eqref{eq:centroid} isolates), and NA is genuinely
higher-order there --- the same reason pairwise negative dependence is strictly weaker than NA for
$n\ge3$, and the reason the exchangeable correlation floor $-1/(n-1)$ tightens away from the
countermonotone $-1$ available only at $n=2$. The counterexample does not survive that tightening: padding
it with an independent uniform (NA by closure, \citealp{joagdev1983}) already drops the ratio below one,
$0.9995$ at $n=3$, and further below for $n=4,5,6$; the independent coordinate dilutes the effect. An
extensive search over exchangeable, moving-average, block, without-replacement, and non-exchangeable
negatively-correlated Gaussian-copula laws at $3\le n\le10$ and every $k$ finds no violation
(\texttt{check\_fan\_conjecture.py}).
\end{remark}

\begin{conjecture}[Per-level contraction, $n\ge3$]\label{conj:disp}
For $n\ge3$, if the calibration scores are negatively associated then $\operatorname{Var}(c)$ is at
most the independent value \eqref{eq:betavar} for each $k$ individually.
\end{conjecture}

Two subclasses settle it outright. The Farlie--Gumbel--Morgenstern copula at negative parameter has
$\operatorname{Var}(U_{(k)})$ a concave quadratic in the parameter with positive slope at
independence, hence below the independent value throughout the NA range; sampling without replacement
reduces, via the same diagonal functional, to a finite centroid inequality that the hypergeometric
single-crossing settles. And in the canonical continuous family the per-level statement is exact, and
more, it is monotone in the correlation.
...
obstruction that blocks localizing Theorem~\ref{thm:agg} to a single $k$, now in closed form. At $n=2$
the LP-violating diagonal is realised by the genuine NA law of Proposition~\ref{prop:counter}, so
\eqref{eq:centroid} fails there; for $n\ge3$ the off-diagonal constraints appear to rule such diagonals
out, and every negatively associated structure tested satisfies \eqref{eq:centroid} with margin.
...
marginals:
\begin{quote}
\emph{Open lemma ($n\ge3$).} Among all $g(u)=\mathbb P(N(u)\ge k)$ realizable by a negatively associated
uniform sample of size $n\ge3$, the independent (binomial) curve maximizes $\operatorname{Var}(c)$.
\end{quote}
The restriction to $n\ge3$ is necessary: Proposition~\ref{prop:counter} realizes an over-dispersing curve
at $n=2$. This is what a total-positivity or coupling argument would have to deliver.
Proposition~\ref{prop:path} and the contest endpoint are the cases settled by hand.
\end{remark}

...
coverage lower bound is then met. We do not claim that randomized APS produces these laws. When
$\alpha > 1/(m+1)$ it cannot produce coverage one, since with distinct scores its coverage is at
most $1 - \alpha + 1/(m+1) < 1$.
...
probability $r = \tau/0.6$, and the second has score $0.9 - 0.3U > 0.6 > \tau$ and is never
included. So the set is $\set{Y}$ with probability $r$ and empty otherwise, every set has at most
...
If labels are unavailable, the mean pairwise product $\E[p_i(X) p_j(X)]$ is symmetric and needs no labeled calibration or evaluation sample. If sets are the object, because a downstream consumer receives sets and nothing else, then
...
randomized APS a perfect classifier's matrix reveals the class proportions. None of this is repaired
by the certificate, which concerns the true label and says nothing about which other labels sit
beside it.

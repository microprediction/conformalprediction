...
similarity, confusion and client quality are read off. We give a specific caution in the form of a no-go theorem and a one bit calculation:
no statistic of the prediction sets determines the confusion matrix once a set of size at least two
occurs, because an unordered set need not retain the ranking that built it, and the label enters
only through calibration. We illustrate with
the conformal correlation matrix of \citet{perlo2026}, the phi coefficient between set membership
indicators, a natural construction that is likely to be reinvented. Two classifiers built inside
randomized adaptive prediction sets, exactly as specified, produce the same matrix while confusing
every class in opposite directions, and seven further hand calculations give a zero entry beside
...
throughout gives the non randomized variant, which is conservative. Examples~\ref{ex:aps} and \ref{ex:privacy} use the randomized variant exactly as defined here.
...
Figure~\ref{fig:chain} places the two losses on the chain of coarsenings that leads from the data to
the matrix.
...
The examples that follow are more concrete, and each contradicts a specific sentence in the paper.
...
proportions $(5/18, 6/18, 7/18)$ with top probabilities $(0.5, 0.6, 0.7)$ both give $p_i = 5\tau/9$
for every class, hence the same law of sets and the same matrix.
\end{example}
...
class'' with probability close to $1-\alpha$. The guarantee \eqref{eq:coverage} is marginal. It does not imply coverage conditional on the input, on a class or on a client. In distribution
free regression with nonatomic covariates, exact pointwise conditional coverage forces essentially
...
table reports accuracy on the non critical classes moving from $0.513$ to $0.494$ on CIFAR-100 and
from $0.859$ to $0.856$ on CIFAR-10, and overall CIFAR-100 accuracy from $0.511$ to $0.493$, under
the conclusion that the method ``does not affect'' non critical performance, without repetitions,
seeds or error bars.
...
marginal coverage lower bound leaves any single entry free among the laws that satisfy it, and by
Theorem~\ref{thm:nogo} no statistic of the sets, this one or any other, determines the confusion matrix once a set of size at least two occurs.
...
\newblock \emph{Journal of the Royal Statistical Society}, 75(6):579--642, 1912.

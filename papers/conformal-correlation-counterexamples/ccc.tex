...
similarity, confusion and client quality are read off. However, the subsequent information gap identity of \citet{cotton2026}, which prices what a
marginal residual law discards under the logarithmic score, has a diagnostic analogue for these
constructions. We give a specific caution in the form of a no-go theorem and a one bit calculation:
...
$f_{(1)}(x) \ge \dots \ge f_{(H)}(x)$, write $r(x, y)$ for the rank of class $y$ in that order, and
...
coverage lower bound is then met. We do not claim that randomized APS produces these laws. It cannot produce coverage one: when
$k \le m$ and the scores are distinct, its coverage is exactly $k/(m+1) < 1$ \citep{romano2020}.
...
using $(Y, C)$ to $\log 2$ using $C$ alone. The matrix is computed from the sets alone. The loss is of the same kind as the
one priced in \citet{cotton2026}, where pooling a conditional residual law into one marginal law
costs exactly the mutual information between residual and input. There the pooled object is the
residual law and the cost is a forecasting regret. Here the pooled object is the joint law of set,
label and ranked prediction, reduced to the law of the set, and the cost is a diagnostic one.
\end{example}

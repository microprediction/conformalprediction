...
and change $Y$ or $\hat Y$, and by Remark~\ref{prop:law} the matrix cannot notice.

The remark also disposes of the claim that the entries ``incorporate the entire distribution
...
With at least three classes, among laws of prediction sets that satisfy the marginal coverage lower
bound \eqref{eq:coverage}, for every $\rho \in [-1, 1]$ there is one with coverage one and accuracy
one whose matrix has an off-diagonal entry equal to $\rho$.
\end{lemma}
...
as under APS. Suppose some set of size at least two occurs with positive probability. Then there are
two such joint laws with the same joint law of $(Y, C(X))$, hence the same law of sets, the same class
...
Two limits on the theorem. First, its hypothesis is sufficient and not necessary. If every set is a
singleton and coverage is one, then $\hat Y = Y$ and the confusion matrix is the diagonal matrix of
the class proportions. With two classes, singleton sets and coverage $\kappa$, the row sums, the
column sums and the trace determine all four entries. The theorem says nothing about cases in which no set of size at least two occurs. Second, the construction changes $\hat Y$ while keeping $C(X)$, and that
...
\draw[arr] (c) -- node[lab, above] {indicators and\\second moments} node[lab, below] {Remark~\ref{prop:law}} (ccm);
\draw[arr] (full) -- (conf);
\draw[arr] (yc) -- (cov);
\end{tikzpicture}
...
dropped, by a different construction: there the ranked prediction is recoverable from the label and
the set, so the first step loses nothing and the second loses the bit. The two results show failures
...
Remark~\ref{prop:law} the law of the sets is all the matrix depends on, so a law is all an
example needs to specify.

In those examples coverage is one, meaning that every set contains the true class. Any marginal
coverage lower bound is then met. We do not claim that randomized APS produces these laws. It cannot produce coverage one: when $k \le m$ and the scores are distinct, its coverage is exactly
$k/(m+1) < 1$, since the rank of the test score among the $m+1$ scores is uniform
...
There are $n$ test inputs. Every set contains the true class $T$, ranked first, and two rare
labels $A$ and $B$ each appear in exactly one set. If they appear in the same set then $z_A = z_B$
...
class'' with probability close to $1-\alpha$. The guarantee \eqref{eq:coverage} is marginal. It does not imply coverage conditional on the input, on a class or on a client. In distribution-free regression with nonatomic covariates, exact pointwise conditional coverage forces essentially
...
The matrix $C_{ab}$ is a conditional mean, so it summarizes rather than determines. A model with $f_B(X) = 0.4$ on every input of class $A$ and a model with $f_B(X)$ equal to $0.6$ or $0.2$ with equal
probability have the same row $(0.6, 0.4)$, and error rates from $A$ to $B$ of zero and one half.
The confusion matrix of the ranked prediction carries that difference and $C_{ab}$ does not, which is why the two are complements.
...
prediction sets and with $C_{ab}$ and $\E[f_i f_j]$ alongside as $\alpha$ varies, are available at
...
Remark~\ref{prop:law} it sees only the law of the sets, by Lemma~\ref{prop:any} the

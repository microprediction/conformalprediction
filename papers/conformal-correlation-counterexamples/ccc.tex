...
The same reasoning gives a no-go result for the whole family of statistics computed from prediction
sets, of which the conformal correlation matrix is one member. Set size, co-membership similarity
between classes, and any correlation of the indicators are others.
...
recover every inclusion rate. Inside randomized APS that is enough to recover the class proportions
exactly.
...
\]
The population matrix determines the true class proportions exactly. For an empirical matrix the
same algebra recovers the empirical inclusion frequencies exactly, from which the proportions are
estimated rather than read off.
\end{example}

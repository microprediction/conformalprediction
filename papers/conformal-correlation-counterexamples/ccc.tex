...
similarity, confusion and client quality are read off. However, the subsequent information gap
identity of \citet{cotton2026}, that a marginal conformal object costs exactly the conditional
information it discards, applies to these types of constructions. We give a specific caution in the form of a no-go theorem and a one bit calculation: no
statistic of the prediction sets determines the confusion matrix once a set of size two occurs,
because the sets are computed without the true label or the ranked prediction. We illustrate with
...
$\pm 0.64$ from pooling two clients with zero correlation on each. For classes that never share a
set the entry is a function of the inclusion rates alone, and when sets have at most one element the
...
the first object. Theorem~\ref{thm:nogo} shows it is already lost at the first step, where the
ranked prediction is dropped. Example~\ref{ex:aps} shows a further loss of one bit at the second
step, where the true label is dropped. The coverage guarantee constrains one scalar of the second
...
column sums and the trace determine all four entries. The theorem says nothing about cases in which
no set of size two occurs. Second, the construction changes $\hat Y$ while keeping $C(X)$, and that
...
carry one full bit. The matrix is computed from the sets alone. The loss is of the same kind as the
...
recover every inclusion rate. Inside randomized APS that recovers the class proportions exactly when every class is retained with
the same probability, which APS does not guarantee in general but which the next construction
arranges.
...
directional. Its row normalized form $\Prob(k \in C(X) \mid Y = y)$ is the conditional version.

...
classes, so the set together with its true label identifies $D$ and $I(D; Y \mid C) = \log 2$. The
sets alone carry no information about the direction of confusion. The sets with their labels carry
one full bit. The matrix is computed from the sets alone.
\end{example}

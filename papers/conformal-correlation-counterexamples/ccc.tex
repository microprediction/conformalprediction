...
classifier confuses. Eight hand-checkable examples show that it is not. Two classifiers built inside randomized
adaptive prediction sets, exactly as specified, produce the same law of prediction sets and the same
matrix while confusing classes in opposite directions, so the matrix does not determine the
confusion matrix. A short no-go theorem extends this to every statistic of the sets: whenever a set
of size at least two occurs, no functional of the law of the sets determines the confusion matrix. The reason is that the matrix depends only on the law of the
prediction set, while confusion depends on the joint law of the set, the true label and the ranked
prediction. Two lemmas give every entry for classes that never share a
set as a function of the inclusion rates alone, and recover every inclusion rate exactly from three
entries when sets have at most one element. Inside randomized APS the second lemma recovers the
class proportions of a client exactly, which bears on the privacy claim made for the matrix.
\end{abstract}

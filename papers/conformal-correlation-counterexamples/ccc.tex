...
Suppose some set of size at least two occurs with positive probability. Then for every functional
$g$ of the law of $C(X)$ there are two joint laws of $(Y, \hat Y, C(X))$ with the same law of sets,
the same class proportions and the same coverage, hence the same value of $g$, whose confusion
matrices differ.
\end{theorem}

\begin{proof}
Let $S$ be a set with $|S| \ge 2$ and $\Prob(C(X) = S) = \delta > 0$, and let $b \ne b'$ be two of
its elements. Take any joint law of $(Y, \hat Y, C(X))$ with the given law of sets in which $\hat Y
\in C(X)$ whenever the set is nonempty, as under APS. Define a second joint law that agrees with the
...
The hypothesis cannot simply be dropped. If every set is a singleton and coverage is one, then
$\hat Y = Y$ and the confusion matrix is the diagonal matrix of the class proportions. With two
classes, singleton sets and coverage $\kappa$, the row sums, the column sums and the trace determine
all four entries. Outside such cases the theorem applies, and Example~\ref{ex:aps} exhibits such a pair of laws

...
point: a serial model fitted on the Gaussianized stream can beat an unconditional
terminal fit when exploitable residual structure remains. The second is that nesting is
...
and Lipschitz budgets multiply (Lemma~\ref{lem:budget}). A fixed increasing link
transports any calibration statement unchanged, because ranks are invariant under it.
...
Drop the distinctness assumption, and condition on the multiset of the $n+1$ values,
which fixes the tie blocks and, under the implementation's rule, the retained knots.
With randomized tie-breaking, ordering tied observations uniformly at random, the
conclusions of Proposition~\ref{prop:sparse} hold conditionally and hence
unconditionally, whatever the tie structure. With the deterministic convention, one
knot per distinct retained value, the conditional bound is $\Delta_n + \beta_n$,
where $\Delta_n$ and $\beta_n$ are the realized largest retained-cell width and
largest tie-block mass, and exactness at retained levels is lost. An unconditional
bound follows by replacing $\Delta_n + \beta_n$ with any almost-sure bound.
\end{corollary}

\begin{proof}
Given the multiset, the position of $Y_{n+1}$ among the $n+1$ slots is uniform, by
exchangeability, and the conditioning fixes every data-dependent quantity: the retained
knots, the cell widths, and the tie masses. Randomized ranks are then uniform on
$\{1, \dots, n+1\}$ regardless of ties, which is the only property the
proposition's proof uses. For the deterministic convention,
$\Pr\{Y_{n+1} \le y_{(r_j)} \mid \text{multiset}\}$ differs from $r_j/(n+1)$ by
at most the tied mass at that knot less one slot, which is at most $\beta_n$, and the
sandwich argument runs conditionally with every cell widened by $\beta_n$. Averaging
over the multiset preserves any almost-sure bound.
\end{proof}
...
grids reported below. Config D0 is the $39$-knot operator followed by the fixed
$N(0,1)$ leaf, the literal conformal terminal forecast, and a full-grid variant retains
every knot. Alternative fixed links replace gaussianize in the E position. Guard and
resolution grids, an iid null suite, and a CRPS-weighted pool complete the set. The
refetched corpus recovers full histories for $572$ of the $701$ series, and the
campaign runs use those, with cluster bootstrap intervals resampling series.
...
The ladder separates three prices. Knot compression is free: the full-grid variant
...
percent, winning $66$ percent of series. The log-score wins are a minority. E beats D0
...
Stopping is one question, and inserting is another. E versus D asks whether the rank
transform is a good architectural stopping rule, and the answer is largely no. The
...
study. Under the log score the operator costs $0.079$ nats against F and wins $9$
percent of series. We return to this in
...
probit of the standard Cauchy distribution function, is indistinguishable from F under
the log score, $0.007$ nats with an interval spanning zero, and captures about half of
the operator's mean CRPS gain. A sinh-arcsinh link with $\delta = 0.5$ pays no
...
same seven chains with the same prior, and it does not merely match: it beats the exact
Bayes pool by $0.021$ nats per observation, winning $76$ percent of series, and wins
the per-series CRPS comparison on $64$ percent. The admission rule is itself a choice
within the grammar, and likelihood weighting is not obviously the right one.
...
nothing to find: the largest E minus D log-score delta across kinds is $0.0013$ nats.
...
grid on the same subsample finds the knot count flat, with $79$, $159$, and the full
grid all within $0.003$ nats of $39$ and intervals spanning zero, while raising the
...
prior. Bootstrap intervals resample series, $2000$ draws, seed $0$.

...
finding we did not anticipate: the largest effect in the study was a mis-typed peer.
A scale rung whose implemented map was not the affine map its inverse assumed had
been bounding the coordinate on the side, crediting the chain without the operator,
and repairing it flipped the operator's measured contribution from a small tax to a
large gain. The grammar's type contracts turned out to be the empirical story, not
only the formalism.
...
F, the same refit without the operator, and here this revision corrects its
predecessors. Earlier drafts measured a log-score tax of about a tenth of a nat for
inserting the operator. The tax was an artifact. The scale rung then in use divided
each emission by a standard deviation that already contained it, a self-normalization
that bounds the emitted coordinate, and a bounded coordinate is most of what the rank
map supplies. The rung was quietly doing the operator's job, so the operator,
measured against it, looked redundant.

With the scale rung repaired to the affine map its inverse applies, the comparison
changes sign and grows an order of magnitude. E beats F by $0.65$ nats on average
...
adaptivity under regime change, and its comparison does not inherit the exact bound.
...
\subsection{An apparent tax, dissolved}\label{sec:smooth}

Earlier drafts reported the opposite insertion result: the operator cost about a
tenth of a nat against F under the log score, and a kernel-smoothed $C^1$ variant of
the operator paid the same, which ruled out the piecewise-linear kinks as the cause.
Both measurements were made against the self-normalized scale rung. The smoothing
test was sound, and its negative answer was the first hint that the tax did not live
in the operator at all. The diagnosis of Section~\ref{sec:interior} completes it:
the tax was a mis-typed peer doing the operator's job, plus about $0.09$ nats of
particle representation error that exact-pullback scoring removes.

A finding those drafts advertised as score dependence, the operator helping CRPS
while costing log score, does not survive the repair. In the repaired chains the
operator helps under both rules, and what remains score-dependent is the narrow
comparison between the rank map and the fixed Cauchy link.

The episode is the grammar's second cautionary tale, beside the warmup emission of
Section~\ref{sec:operator}. A mis-typed rung, one whose implemented map is not the
map its inverse assumes, transfers credit between its neighbors and can misprice an
operator two positions away. The type contracts are not pedantry. They are what the
measurement of any one operator silently assumes about all the others.
...
representation price the exact-pullback runner measures in full. The certificate's resolution parameter is not the empirical bottleneck. The
...
load-bearing under both scoring rules, and the tax earlier drafts measured was a
mis-typed peer doing its job, plus representation error. A pool that admits the

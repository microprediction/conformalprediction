...
transforms. Interpolated, it is invertible and, on distinct exchangeable input with its slope
guard inactive, exact at retained levels, within one knot cell everywhere. The
certificate attaches to the operator's immediate output and is not inherited
automatically. On economic series a serial model after the operator outscores the
literal stop on average, while against the same model without it, CRPS improves and
log score worsens. A Bayes pool admits it at bounded cost.
\end{abstract}
...
predictive systems made the map emit a distribution function, and conformal calibrators
\citep{vovk2020calibrators} apply that map to another system's output, one level of
composition with the map still the last word. We propose a different organization, in
...
skeptic of the standard conformal prediction patterns might reasonably ask why an
approach that essentially adopts the empirical distribution, in a sense advocating not
modeling at all, is appropriate in the context of model residuals but not in general
(for instance, as a replacement for the base model).
...
is what one settles for. That answer does not carry over to a forecaster scored by a
...
modeling decision, not a consequence of the guarantee, and pooling the observable
itself, the limiting case of an inadequate model, would frequently be ludicrous.
...
The declaration also has a price, and three objects need separating. The exact-knot
interpolant is what the propositions of Section~\ref{sec:typing} certify. The
slope-guarded map is what the experiments run: the cap is enforced by lowering any
knot whose segment is too steep, so when it binds, the map no longer passes through
the displaced order statistics, and retained-level exactness is suspended there. The
...
sensitivity budget and to expose, through the rank-displacement term of
Proposition~\ref{prop:sparse}, the finite-sample calibration price paid for
enforcing that budget. The canonical guard would be the least-squares projection onto
the slope-constrained set, the regularized transport of \citet{paty2020regularity}.
A guard fitted to this paper would instead minimize the rank displacement $e$ subject
to the budget, solving exactly the optimization the proposition prices. Both are
refinements of the implemented walk, and the deletion variant of
Remark~\ref{rem:guard} is the one we measure.
...
not be small, as the next remark shows, so the guarded operator is not
$\Delta_n$-conformal unconditionally. The $e$ term is also the price a future
bounded-memory compaction would pay. Ties need their own statement.
...
Take $n = 78$ calibration values, seventy-one at $0, 1, \dots, 70$ and eight near
$10^9$, all distinct, in exchangeable order. With $39$ retained knots the top knot is
the seventy-eighth order statistic and $\Delta_n = 2/79$. The slope cap, at twenty
times the central inverse slope, drags that knot from the high cluster down to roughly
$671$, between the clusters. At the retained level $78/79$ the probability that a new
exchangeable draw falls below the displaced knot is $71/79$, an error of $7/79$,
exceeding $\Delta_n$. Retained resolution and tie mass are therefore not the whole
price of the certificate. Guard displacement is a third term, zero exactly while the
guard sleeps. Deleting offending knots instead of moving them restores exact order
statistics at an enlarged $\Delta_n$, and the operator implements it as an option.
On the campaign corpus the deletion guard is indistinguishable from the moving cap,
identical on all but three percent of series with a mean difference under $0.0003$
nats, so the unconditional certificate costs nothing measurable.
\end{remark}
...
The narrowing matters, because previsibility is not a substitute for splitting.
Previsibility prevents leakage, the map at time $t$ not containing $y_t$, and that is
all it prevents. It does not make the transformed stream exchangeable: previsible
...
The dropped series carried a larger share of the operator's log-score tax on the
original run, which is part of why the tax reads $0.079$ nats here against $0.108$
there, the rest being data vintage. The corpus can always grow.
...
Taking $Q$ to be the sub-pool without the gaussianize chains, the total price of
admitting them is at most $\log(1/\pi_Q)$ nats, the prior mass of the retained block
rather than of the admitted one, however the data falls, and the per-observation price
vanishes. The proposition applies
...
Rerunning both pools as genuine Bayes mixtures, unclamped, unpruned, with the
reference blend applied inside the weight updates and the depth prior applied once,
reproduces the comparison on a fresh data vintage: G ahead of H by $0.012$ nats on
average over $572$ series, the same lopsided shape, wins on one series in ten, median
difference zero. The telescoping identity applies verbatim to these scores. The
sub-pool prior mass is $\pi_Q = 0.573$, so the proposition's worst case is $0.56$
nats in total however long the series run, and the realized comparison sits on the
profitable side of it. Nothing
...
Writing them this way shows what the grammar adds. Each system holds the rank map at a
...
level, driven by coverage. None fits a model on the map's output, none carries
predictive densities back through its inverse, and none scores the map as one operator
among peers under a proper scoring rule. Those are the moves this paper adds, and the
...
seventy-odd series. The deletion-guard comparison is the \texttt{delguard}
subcommand, using the operator's \texttt{guard} option, on the full campaign corpus.

...
guard sleeps. Deleting offending knots instead of moving them would restore exact
order statistics at an enlarged $\Delta_n$, and is the cleaner operator we did not
implement.
\end{remark}
...
Rerunning both pools with untempered weights and the depth prior applied once, with
the runner that ships alongside the benchmark, reproduces the comparison on a fresh
data vintage: G ahead of H by $0.011$ nats on average over $572$ qualifying series,
the same lopsided shape, win rate under a third, median difference zero. That runner
is near-Bayes rather than exact. The shared ensemble clamps each expert's log density
to $[-20, 20]$ before the weight update, prunes the emitted mixture to twenty
components, and applies the scoring blend toward the reference outside the updates,
so the telescoping identity does not apply verbatim to the reported scores, and the
rerun stands as a robustness check rather than an instantiation of the bound. Nothing

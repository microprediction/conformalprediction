...
Viewed this way conformal prediction is not, in general, an alternative to statistical
modeling put to one side but a gracefully integrated operator in a chain whose finite-sample properties
may well be valuable at the point where they are computed. This distinguishes it from other
operators that it gracefully commingles with, but it does not privilege stopping there, nor need
it remove the modeling that follows where appropriate. Used carefully in this manner, it
does not materiallty degrade predictive quality under a proper scoring rule, at least in our experiments.

The standard conformal pattern also carries a tension the present work avoids. A
skeptic of the standard conformal prediction patterns might reasonably ask why
 heavy anchoring on the empirical distribution (in a sense, thereby not
modeling at all) is appropriate in the context of model residuals, whereas to apply the same
 emphasis in general would be absurd.

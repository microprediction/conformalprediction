...
the chain. On the economic series studied, the conformal step earns its
place under both log-likelihood and CRPS, though a fixed heavy-tailed link captures
most of the same benefit without the certificate.
\end{abstract}
...
where it is a bad choice. We confirm both on a corpus of economic series and report a third
finding we did not anticipate: the operator's measured value is dominated by whether
any peer in the chain already bounds the emitted coordinate. An ablation with a
self-normalized scale rung, which bounds on the side, absorbs nearly all of the
operator's contribution, so the grammar's type contracts are the empirical story,
not only the formalism.
...
bandwidth, represented by a monotone $C^1$ cubic through the same knots, and Section~\ref{sec:robust}
checks that smoothness changes little.
...
past its budget. The lemma budgets the ideal pulled-back predictive. The
implemented nine-particle mixture approximates that predictive, and the
approximation error is not bounded here. The global $L$ is also purchased with the knot displacement of
...
dial. The budget statement is for the piecewise-linear operator: the
Fritsch--Butland tangents of the smooth variant can steepen the inverse of a capped
secant by a factor approaching two, so that variant's declared budget is $2L$. Fitting the tail where the sample reaches, generalized-Pareto by censored maximum
...
the certificate is $\delta$-relaxed and assumption-laden, which is where the
fixed-link comparisons of Section~\ref{sec:interior} bite.
...
A second campaign adds the decomposition ladder and robustness
grids reported below. Config D0 is config D with its fitted leaf replaced by the fixed $N(0,1)$ law, the
terminal conformal forecast up to the nine-particle inverse representation, and a
full-grid variant retains every knot. Alternative fixed links replace gaussianize in the E position. Guard and
...
The dropped series are mostly the short monthly ones. The corpus can always
grow.
...
\caption{Five comparisons on the $572$-series campaign corpus, under both scoring
rules, with the repaired affine scale rung. Bars to the right favor the first-named
config. Log score improvement is in nats per observation. CRPS improvement is the
mean per-series ratio, expressed as the percent cut. Win rates are
...
$N(0,1)$ law at the operator's output. Its scored predictive is the nine-particle
representation of that law's pullback, so it is the terminal conformal forecast up
to inverse representation. Config E reads
...
The ladder, on the $572$-series campaign corpus, separates three prices. Knot
compression is free: the full-grid variant differs from $39$-knot D0 by $0.003$
nats, compressed ahead. Refitting the terminal leaf hurts: D scores $0.011$ nats
below D0, winning $19$ percent of series, because the operator has already
standardized its output and a fitted variance adds estimation noise. Serial
refitting beats the literal stop on average: E improves on D0 by $0.038$ nats
(interval $0.027$ to $0.052$) and cuts the mean per-series CRPS ratio by $3.2$
percent, winning $59$ percent of series on CRPS. The margin is skewed. The median
log-score difference is a thousandth of a nat with wins on $52$ percent of series,
and the gains concentrate where serial structure is strong. On the iid null suite of
Section~\ref{sec:robust} the same comparison runs the other way, by $0.011$ to
$0.051$ nats depending on the null, which is the estimation cost of refitting when
there is nothing to find, and it makes the corpus margin read as signal.
...
gaussianize scores $-2.27$ nats and $1.24$ CRPS. Refitting AR(1) through it scores
$-2.00$ and $0.95$, beating both the stop and the same AR chain without the operator
($-2.13$, $1.04$). When the downstream learner is well specified only in the empirical map's
...
rank transform is a good architectural stopping rule. On average the answer is no.
On the median series it is a coin flip. The operator's incremental value is E versus
F, the same refit without the operator. E beats F by $0.65$ nats on average
(interval $0.54$ to $0.76$), with a median of $0.06$, wins on $61$ percent of
series, and wins the per-series CRPS comparison on $97$ percent. Without a bounding
rung, heavy-tailed coordinates reach the serial model and its leaf raw, and the
worst series destroy the chain. In this chain the operator is not decoration. It is
the difference between a usable pipeline and a fragile one.

The comparison is sensitive to the peers' types, and one ablation shows how sharply.
Replace the affine scale rung by a self-normalized one, dividing each emission by a
running standard deviation that already contains it. The emitted coordinate is then
bounded by construction, the chain without the operator inherits the bound, and the
measured value of inserting the operator collapses to a small negative number.
Bounding the coordinate is most of what the rank map supplies, and any rung that
bounds on the side absorbs the operator's credit. A mis-typed rung, one whose
implemented map is not the map its inverse assumes, can misprice an operator two
positions away. The type contracts are what the measurement of any one operator
silently assumes about all the others.

Part of every particle-scored number is representation rather than operator. The
exact-pullback runner scores the true change of variables,
$\log q(m(y)) + \log m'(y)$, with analytic Jacobians and no particles. The
nine-particle representation costs about $0.09$ nats on the gaussianize chains,
roughly constant across configs, and exactly zero on F, whose chain is affine and
whose mixture pullback is already exact. Scored exactly on both sides, E beats F by
$0.74$ nats and beats D0 by $0.032$, so the ladder's conclusions are about the
operator, not about its representation.

The alternative links complete the picture. A fixed Cauchy link in the same
position beats F by $0.69$ nats, and it beats the rank map itself by $0.039$ nats on
average, winning $76$ percent of series, while the rank map wins the per-series CRPS
comparison against it on $66$ percent. A sinh-arcsinh link at $\delta = 0.5$ wins
$83$ percent of series against the rank map and loses the mean to its own tail
blowups, and the Yeo--Johnson links, which do not bound the tails at all, are
destroyed with F. What the corpus rewards is heavy-tailed monotone bounding. The
rank map supplies it adaptively, with a certificate. A fixed Cauchy link supplies it
a shade better on average, with no certificate at all.

\subsection{Nesting limits the cost of admission}\label{sec:nesting}

The raw conformal pattern C is a reasonable forecaster on this corpus, better than
the location baseline on average under both scores. It is also, on a minority of
series, a bad choice. Config G, the pool over all seven chains, gaussianize chains
included, beats the fixed pattern C on $87$ percent of series by log score, by
$0.163$ nats on average. On the twenty series where C is worst, with relative CRPS
$1.018$, the pool sits at $0.824$ on the same series. The pool walks away from the
production rule where it underperforms.

Admitting the pattern now buys a real gain, and the shape is still lopsided. The
exact Bayes pool G beats its sub-pool H, the same pool with the gaussianize chains
removed, by $0.093$ nats per observation on average (interval $0.072$ to $0.115$)
and by $4$ percent of the mean per-series CRPS ratio. The median difference is zero
to four decimals and G wins $36$ percent of series by log score: on most series the
admitted chains change nothing, and where the repaired F-chains are fragile they
change everything. Frequent negligible differences buy occasional rescues. Under
...
block, not of the admitted one. The reported pools are exact Bayes mixtures, so the
proposition applies to them verbatim: no likelihood clipping, no cross-expert
mixture pruning, the reference blend inside the weight updates, and the depth prior
applied once. The operator's $|z| \le 8$ learner-input channel stays active inside
each expert, as everywhere in the paper. The production ensemble instead tempers its
likelihood update at rate $0.8$ and applies its depth penalty per step, for
adaptivity under regime change. Its comparison does not inherit the exact bound, and
it shows the same picture, the tempered G ahead of its H by $0.078$ nats.
The runner resets the mixture weights to
the depth prior at the start of the scored window, because the bound's prior-mass
penalty controls the loss from initialization, not from an arbitrary suffix: scoring
after a burn-in that already reweighted the pool would need the realized posterior
mass in place of the prior. With the reset, the sub-pool prior mass
$\pi_Q = 0.573$ makes the worst case $0.56$ nats in total for the reported scores,
however long the series run, and the realized comparison, $0.093$ nats per
observation in G's favor, sits far on the profitable side of it.
...
same seven chains with the same prior. Its comparison with the exact Bayes pool
is mixed and close. The CRPS-weighted pool wins $65$ percent of series by log score
and $68$ percent by CRPS, with median CRPS ratio $0.99$, while its mean log score is
$0.006$ nats lower with an interval spanning zero. The admission rule is itself a
choice within the grammar, and neither weighting dominates the other here.
...
Three checks bound the sensitivity of these findings. An iid null suite runs the ladder when there is nothing to find. The kinds are
Gaussian, Student $t_3$, lognormal skew, and a two-component mixture, forty
replicates each. Against the literal stop the refit loses on every null, by $0.011$
to $0.051$ nats depending on kind, which is the estimation cost of fitting absent
structure and is the sign the corpus reverses. Against the fitted leaf the refit
gains up to $0.023$ nats on the skewed and mixture nulls, so part of any
E-versus-D margin is adaptation rather than serial structure, and the stopping
comparison is E versus D0 for exactly this reason.
A guard grid crossing slope caps $\{10, 20, 40\}$ with clamps
$z_{\max} \in \{6, 8, 10\}$ on a hundred-series subsample moves config E by
$0.003$ nats end to end, so the guard constants are not doing the work. A resolution
grid on the same subsample finds the knot count flat in the point estimates: $79$,
$159$, and the full grid all sit within $0.003$ nats of $39$, none ahead of it. On a
hundred series those intervals cannot exclude effects of the size the particle count
shows. Raising the inverse from nine particles to nineteen is worth $0.016$ nats,
and going further is not, which is the subsample's view of the $0.09$-nat
representation price the exact-pullback runner measures in full. The
kernel-smoothed $C^1$ variant of the operator tracks the linear one, $0.003$ nats
below it on the refit chain, so smoothness is not where the value lives. The
certificate's resolution parameter is not the empirical bottleneck. The inverse's
resolution is, mildly.
...
average. Inserted into a chain whose scale rung is honestly affine, the operator is
load-bearing under both scoring rules, and its measured value collapses only when
some other rung bounds the coordinate on the side. A pool that admits the
pattern pays a bounded price and collects a real gain. A fixed Cauchy link supplies
the same bounding a shade better on average, and carries no certificate.
...
part of the invertible map. The nine-particle mixture representation of the pullback
is a fourth: the campaign scores that mixture exactly, which is exact scoring of an
approximate distribution, not exact change of variables, and its representation error
is separate from all three prices above. Remark~\ref{rem:guard} prices the middle one.
...
Let $Y_1, \dots, Y_N$ be exchangeable, $N = n + 1$, and let the knot construction
depend on its calibration values only through their multiset, as the sorted-buffer
operator does after warmup. Condition on the multiset $M$ of the $N$ values. For each
slot $s$, build the map $m_s$ from the other $n$ values, and suppose its retained
knots satisfy the bracketing contract with rank displacement at most $e_s$ and
retained-cell width at most $\Delta_s$. Let $\Delta^*$ and $e^*$ be the maxima over
slots, and $\beta^* = (b^* - 1)/N$ with $b^*$ the largest tie-block size of $M$.
Let $S$ be the held-out slot, uniform given $M$ by exchangeability, and
$U = \Phi(m_S(Y_S))$. Then, almost surely,
\[
  \sup_u \big| \Pr\{U \le u \mid M\} - u \big| \;\le\;
  B(M) := \Delta^* + \frac{e^* + 1}{N} + \beta^*,
\]
and unconditionally
$\sup_u |\Pr\{U \le u\} - u| \le \mathbb{E}\,[B(M)] \le
\operatorname{ess\,sup} B(M)$.
\end{proposition}

\begin{proof}
Fix $u \in (0,1)$. For slot $s$, let $a_s \le Nu \le b_s$ be the retained ranks of
slot $s$'s grid adjacent to $Nu$, with the conventions $r_{s,0} = 0$ and
$r_{s, K_s + 1} = N$, so that $(b_s - a_s)/N \le \Delta^*$, and this holds even
when the grids differ across slots, as under the deletion guard. Set
$a = \min_s a_s$ and $b = \max_s b_s$; then $a/N \ge u - \Delta^*$ and
$b/N \le u + \Delta^*$. For slot $s$ the event $\{U \le u\}$ is
$\{Y_s \le m_s^{-1}(\Phi^{-1}(u))\}$, and monotonicity places
$m_s^{-1}(\Phi^{-1}(u))$ between slot $s$'s knots at ranks $a_s$ and $b_s$, with the
convention that the interval is unbounded on a side where the boundary rank is $0$ or
$N$. Removing one value moves order statistics by at most one position,
$Y_{(k)} \le y^{(s)}_{(k)} \le Y_{(k+1)}$, so the bracketing contract traps those
knot values between $Y_{(a - e^*)}$ and $Y_{(b + e^* + 1)}$. At least $a - e^*$ of
the $N$ values are at most $Y_{(a - e^*)}$, and at most $b + e^* + 1 + (b^* - 1)$
are at most $Y_{(b + e^* + 1)}$. The count of slots satisfying the event therefore
lies between $a - e^*$ and $b + e^* + 1 + (b^* - 1)$, and dividing by $N$,
$\Pr\{U \le u \mid M\}$ differs from $u$ by at most $B(M)$. Taking expectations
gives the marginal statement.
\end{proof}
...
$\Delta_n + e/(n+1)$ with the smallest $e$ feasible for the fixed retained grid.
When flat knots are dropped the grid changes, so $\Delta_n$ in the bound is the
width recomputed after the drop, and minimizing $e$ alone need not minimize the
total: the guard fitted exactly to the certificate would minimize
$\Delta_n + e/(n+1)$ jointly. On this example the per-slot smallest displacement is
$e = 7$ and the leave-one-out maximum of Proposition~\ref{prop:loo} is $e^* = 8$.

Three certificates must be kept apart. Budgets $e_0$ and $\Delta_0$ declared in
advance give a prospective certificate, available before the test value arrives.
The leave-one-out maxima $e^*$ and $\Delta^*$ give the conditional certificate of
Proposition~\ref{prop:loo}, and computing them needs the completed multiset and $N$
reconstructions, so it is an ex-post transductive diagnostic. The single realized
$e_S$ and $\Delta_S$ that a guard reports on the forecasting path bound neither and
are a diagnostic only.
On the campaign corpus, replacing the moving cap by either guard moves mean log score
by less than $0.0003$ nats, with identical output on eight in ten series.
\end{remark}
...
Every number in Section~\ref{sec:experiments} is produced by one of four runners
in the \texttt{benchmarks} directory of the \texttt{skaters} repository. They are
\texttt{gaussianize\_chain.py} and \texttt{grammar\_campaign.py}, with
\texttt{grammar\_exact\_bayes.py} and \texttt{grammar\_exact\_pullback.py}
described below. The original run
...
The scale operator is an EWMA standardization with rate $0.05$, emitting against
the prior state so that the rung is the affine change of coordinates its inverse
applies, with the first informative emission scaled to one as cold start. The AR(1) operator
...
inverse slope and the downstream-update clamp $|z| \le 8$. The implementation
returns the clamped value from the forward callable, so the software object is not
itself injective: the invertible map and the bounded update channel are separated in
the analysis, and a typed implementation would return them as two channels.

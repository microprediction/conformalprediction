...
operator. We treat it as one operator in a grammar of composable transforms.
Interpolated, the rank map is invertible. On distinct exchangeable input it is exactly
calibrated at its retained levels, and within one knot cell everywhere, while its
slope guard sleeps. The certificate covers the operator's output, nothing downstream.
On economic series, refitting a serial model after the operator beats stopping there
on average. Inserting the operator into the same chain improves CRPS and worsens log
score. A Bayes pool admits it at bounded cost.
\end{abstract}
...
once, and the map is still the last word. We propose a different organization, in
...
thirty-five of the thirty-nine knots are untouched, and the worst segment sits
exactly at the cap. On the campaign corpus both are indistinguishable from the moving cap,
...
data vintage. Nothing is clamped or pruned, the reference blend sits inside the
weight updates, and the depth prior is applied once, so the telescoping identity
applies verbatim to these scores. G is ahead of H by $0.012$ nats on average over

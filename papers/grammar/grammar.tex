...
operator. The factorization is a chain in a grammar of transforms, the rank map one
operator beside location, scale, and autoregression. Interpolated, it is invertible.
On distinct exchangeable input it is exactly calibrated at its retained levels,
within one knot cell everywhere, while its guard sleeps. The certificate covers the
operator's output, nothing downstream. On economic series, a serial refit after the
operator beats stopping there on average. Inserting it into the same chain improves
CRPS and worsens log score. A Bayes pool admits it at bounded cost.
\end{abstract}
...
order statistics. Appendix~\ref{app:certificate} separates the objects involved,
prices the displacement, and traces the guard's ancestry in Lipschitz isotonic
regression and regularized empirical transport.
...
Compression, a binding guard, and ties each pay a stated price, and a leave-one-out
statement covers guards whose budgets are data-dependent.
Appendix~\ref{app:certificate} states and proves the accounting, with a
counterexample showing the moving cap can dominate the other prices. Nothing
downstream depends on the details, only on the headline: the certificate is priced
in the operator's computational budget, and none of the prices moved the campaign's
scores.
...
This appendix carries the exact calibration accounting for readers who want
coverage statements. The rest of the paper uses only its headline.
...
There is a subtlety the guards force into the open.
Proposition~\ref{prop:sparse} needs a budget $e$ declared for every possible sample.
...
tie mass, and guard displacement: the guarantee is priced in the operator's

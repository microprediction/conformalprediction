...
def covid_table():
    ...
    out.append(f'<p>At the joint level, the CF-RNN interval is the calibration maximum at every horizon and reaches {100*g("CFRNN_bonf","joint"):.0f}% joint coverage, above the 90% target, while laplace&rsquo;s 99.8% quantiles from each area&rsquo;s own hundred days reach {100*g("laplace_bonf","joint"):.0f}%. By the paper&rsquo;s criterion CF-RNN wins. By the interval score at the level both intervals claim, laplace&rsquo;s intervals score {g("laplace_bonf","IS"):,.0f} against {g("CFRNN_bonf","IS"):,.0f}, {100*(1-g("laplace_bonf","IS")/g("CFRNN_bonf","IS")):.0f}% lower, with no training data and no other areas. The constant width is wide where the area is quiet and still misses by hundreds of cases where the January wave is steep, and the score charges the misses at \\(2/a = 1000\\) times their size.</p>')
    out.append(f'<p>At the per-horizon level, MQ-RNN, which the paper reports as failing, has the best interval on the table, {g("QRNN","IS"):,.0f} against {g("CFRNN_marg","IS"):,.0f} for the uncorrected CF-RNN and {g("laplace_marg","IS"):,.0f} for laplace. Its coverage is {100*g("QRNN","cov"):.0f}% at a 90% target and the score still prefers it, because its misses are small. The dropout RNN at {100*g("DPRNN","cov"):.0f}% coverage scores {g("DPRNN","IS"):,.0f}. The area series are synchronous, every test area&rsquo;s forecast window is the same seven weeks of the January 2021 wave, and a model trained on other areas over the same calendar has seen the shape of the wave. That is real information, it is what the paper&rsquo;s Figure 1 argues for, and the interval score credits the quantile RNN for using it. Laplace, which sees one area and no calendar, is not competitive at this level and we do not claim otherwise. The uncorrected CF-RNN interval adds a constant to the same LSTM point forecast that MQ-RNN refines with two trained quantiles, and the constant is what costs it.</p>')
    out.append("<!-- /COVID -->")
    return "\n".join(out)

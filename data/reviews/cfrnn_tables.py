...
def fred_table():
    ...
    out = [f'<!-- FRED -->\n<p>The skaters benchmark harness runs laplace, a zero-dependency online distributional forecaster, on the FRED daily universe of change series. For this study every series gets laplace at horizon \\(H = 10\\), and at each origin two sets of intervals are built from the same point forecasts, laplace&rsquo;s predictive means. The CF intervals add the CF-RNN quantile of the last 500 resolved absolute residuals at each horizon. The laplace intervals are the quantiles of laplace&rsquo;s own predictive distribution at the same levels. Both are scored at \\(a = \\alpha/H = 0.01\\), the Bonferroni level with joint target 90%, and at \\(a = \\alpha = 0.1\\) per horizon. {N} series, a median of {med([r["n_origins"] for r in rows]):.0f} scored origins per series, scored on the most recent 2,000 changes after a 300-step warm-up. The universe is every cached daily series that clears the harness&rsquo;s two standing screens, at least 500 changes and no single move dwarfing the series&rsquo; own scale, which is 569 of the 701 cached. Scores are on each series&rsquo; own scale, so the table reports medians over series and the share of series on which each interval scores better.</p>\n',

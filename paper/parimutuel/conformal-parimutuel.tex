...
information the crowd's price $q$ is missing about the truth $p$. When the crowd prices the
marginal, that is, offers fair odds, the averaged growth of an entrant with belief $b$ is
$I(R;X)-\mathbb E_X\,\mathrm{KL}\big(g(\cdot\mid X)\,\|\,b(\cdot\mid X)\big)$. \citet{kemp2022}
write exactly this decomposition, $\gamma=I(E;S)-\mathbb E_s\,\mathrm{KL}\big(P(E\mid s)\,\|\,
X(E\mid s)\big)$, in a population-growth setting unconnected to conformal prediction; the next
section is the observation that a conformal predictor is precisely such a fair-odds crowd.
...
\end{equation}
Flat pricing on the rank scale is fair odds: the implied price equals the true marginal of $U$,
which is what marginal calibration means. That is the hypothesis under which side information is
worth its mutual information, so the rent is settled before we compute it. Two entrants step up.
...
Let $\operatorname{HSIC}(U,X)$ use a bounded product kernel, $k\le K$ on the product space. Then
\begin{equation}
...
\begin{proof}
Write $\operatorname{MMD}=\operatorname{HSIC}^{1/2}=\sup_{\|f\|_{\mathcal H}\le 1}\big|\mathbb
E_{P_{U,X}}f-\mathbb E_{P_U\otimes P_X}f\big|$, the integral probability metric over the unit ball
of the tensor RKHS. A unit-ball witness satisfies $|f(z)|=|\langle f,k(z,\cdot)\rangle|\le
\sqrt{k(z,z)}\le\sqrt K$. With total variation $\mathrm{TV}=\tfrac12\int|\mathrm dP_{U,X}-\mathrm
dP_U\,\mathrm dP_X|$,
\[
\big|\mathbb E_{P}f-\mathbb E_{Q}f\big|\le\|f\|_\infty\!\int|\mathrm dP-\mathrm dQ|\le 2\sqrt K\,
\mathrm{TV},\qquad\text{so}\quad \operatorname{MMD}\le 2\sqrt K\,\mathrm{TV}.
\]
Pinsker's inequality \citep{cover2006} gives $\mathrm{KL}(P_{U,X}\|P_U\otimes P_X)\ge 2\,
\mathrm{TV}^2$, and that KL is $I(U;X)=I(R;X)$. Combining,
$I(R;X)\ge 2\,(\operatorname{MMD}/2\sqrt K)^2=\operatorname{HSIC}/(2K)$.
\end{proof}

Distance covariance \citep{szekely2007} is a fixed multiple of this HSIC \citep{sejdinovic2013}, so the companion
detector inherits the same bound and a measured value certifies a minimum wasted log-score, hence
by Theorem \ref{thm:rent} a minimum extractable rent. The matching upper bound does not exist with
a fixed kernel, since MMD metrizes weak convergence \citep{simongabriel2023} while KL does not: a
sequence of laws can have $\operatorname{HSIC}\to 0$ with $I(R;X)\to\infty$. Under a bounded
conditional density ratio $g(u\mid x)\le M$ one has the crude $I(R;X)\le\log M$, refinable through
$\chi^2$ \citep{wangtay2022}, but the exact value needs a different object.
...
The gap $\Delta=I(R;X)$ is the companion paper \citep{cotton_marginally}; the distance-covariance
diagnostic and the signed de Finetti reading are \citep{cotton_signed}. That optimal log-growth with
side information equals the mutual information is classical \citep{kelly1956,cover2006,
barroncover1988}, and \citet{kemp2022} write the fair-odds form $\gamma=I-\mathbb E\,\mathrm{KL}$
that Lemma~\ref{lem:growth} specialises, for population growth rather than prediction. Log-loss as
negative log-wealth is the prequential and game-theoretic tradition \citep{dawid1984,shafervovk2019},
and the growth-optimal e-variable's identity with a KL is from safe testing and the e-value calculus
\citep{gruenwald2024,vovkwang2021}; conformal test martingales are \citet{vng2003}.

Closest are \citet{arnold2023}, who build e-processes for marginal PIT calibration, and
\citet{shaer2023}, who test conditional independence by betting --- the mechanics of
Proposition~\ref{prop:eprocess}(i) --- but neither identifies the growth rate with $I(R;X)$ or frames
it as the conformal rent. The parimutuel mechanism and its log-optimal incentive are
\citet{cotton_micro2022,cotton_horserace,micro_density}. The contribution is the assembly: a
self-contained parimutuel derivation of $I(R;X)$ in the conformal idiom, the recognition that
conformal calibration is the fair-odds condition under which side information is worth its mutual
information, the identification of the gap with the rent in a deployed market, and the pairing of a
certified lower bound with an anytime-valid growth-rate estimate.
...
\bibitem[Kemp and Bettencourt(2022)]{kemp2022} Kemp, J.~T. and Bettencourt, L.~M.~A. (2022). The
Bayesian origins of growth rates in stochastic environments. Preprint, arXiv:2209.09492.

...
concave (equivalently $-\min(j,n-m)=\max(-j,m-n)$ is convex and nondecreasing). \citet{shao2000}
compares a negatively associated sum to its independent counterpart in convex order,
$N(u)\le_{\mathrm{cx}}\mathrm{Bin}(n,u)$, so $\mathbb E[\phi(N(u))]\le\mathbb E[\phi(\mathrm{Bin})]$ for
convex $\phi$ and the reverse for concave; applied to $m\mapsto\min(j,n-m)$,

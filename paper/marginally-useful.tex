...
it. The familiar cautions about conformal prediction then follow as context rather than
discoveries: marginal coverage is not conditional (and distribution-free conditional
coverage is impossible without structure), validity is insensitive to sharpness, and the
guarantee requires exchangeability. On a coverage--score plane, conformalizing a fixed
model is a purely horizontal move, toward correct coverage and never toward a better score.
The same decomposition marks the converse: when coverage itself is the objective, the gap is
irrelevant and conformal prediction is exactly right. All figures are reproducible.
\end{abstract}
...
failure. A more useful diagnosis, we think, is a \emph{category error}: conformal
...
\begin{itemize}
  \item \emph{Orthogonality} (Section~\ref{sec:orthogonality},
  ...
  \item \emph{The residual-information gap} (Section~\ref{sec:gap},
  ...
  \item \emph{The coverage--score plane} (Section~\ref{sec:plane}). A two-axis
...
\section{Three limits of the marginal guarantee}\label{sec:marginal}
...
and a nonconformity score. The point is conditional on the score: it is sharp
when the density is \emph{not} used to build $s$ (as for the canonical residual score),
and it does not apply to density- or quantile-based scores.
...
\section{Re-leveling, and the residual-information gap}\label{sec:gap}
...
\begin{enumerate}
\item[\emph{(A)}] \emph{Signed-residual CPS.} The CPS predictive distribution is
...
\item[\emph{(B)}] \emph{Absolute-residual intervals.} Ordinary split conformal at
...
\begin{itemize}
  \item \emph{Residual split conformal is a horizontal projection.} It maps a report
  ...
  \item \emph{Conformal predictive systems are different:} they supply their own
  ...
  oracle minus $I(R;X)$.
  \item \emph{Recalibration and better modeling move vertically.} Fitting shape to a
...
guaranteed.
\begin{itemize}
  \item \emph{Weighted conformal prediction} \citep{tibshirani2019conformal} restores
  ...
  \item \emph{Adaptive Conformal Inference} \citep{gibbs2021adaptive} updates the level
  ...
  \item \emph{EnbPI} \citep{xu2021conformal} drops exchangeability for approximate
...
ideal:
\begin{itemize}
  \item \emph{Selective prediction and risk control.} ``Return a small set guaranteed
  ...
  \item \emph{Retrieval and shortlisting,} where the product is a candidate set and
  coverage is recall.
  \item \emph{Anomaly and novelty detection via conformal $p$-values}, conformal
  ...
  \item \emph{Compliance and long-run frequency control,} where a certificate of the

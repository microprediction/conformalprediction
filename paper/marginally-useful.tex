...
\begin{abstract}
Conformal prediction supplies a genuine guarantee: under exchangeability it gives
finite-sample, distribution-free \emph{marginal} coverage for a set. We argue that this
is often misread as evidence of distributional forecast quality, and we make the
separation precise. After reviewing why the marginal guarantee is weaker than it looks
(it is not conditional, conditional coverage is unavailable distribution-free without
structure, and it requires exchangeability), we give three results for a fixed location
predictor with a residual score. (i) The conformal set is determined by residual order
statistics, so its coverage can be held fixed while an accompanying density's log-score
is sent to $-\infty$. (ii) The log-score regret of a single-shape residual predictive
system relative to the oracle is the mutual information $I(R;X)$ between residual and
input, which no recalibration that ignores $X$ can reduce. (iii) A coverage--score
diagnostic displays conformalization as a horizontal move that corrects coverage without
improving sharpness. We close with the converse: when coverage \emph{is} the objective,
conformal prediction is exactly right. All claims are illustrated by reproducible figures.
\end{abstract}
...
gives the converse --- when coverage really is the objective ---
...
the sample size, noise level, and $\alpha$ are varied. We do not dispute any of this. We
dispute what it is taken to mean.
...
be closed for free, and both surface as concrete finite-sample facts.
Part~(a) is illustrated in Figure~\ref{fig:nogo}: localizing split conformal to recover
...
of the lemma, arriving as arithmetic, so that uniform coverage is recovered only as the
fraction of infinite-length intervals grows. Part~(b) is Figure~\ref{fig:subgroup}: the
distribution-free protection of all $\delta$-subgroups is exactly the flat band at level
...
\caption{The price of conditional coverage (Lemma~\ref{lem:nogo}a). As the conditioning
...
\caption{Subgroup coverage buys only a wider band (Lemma~\ref{lem:nogo}b). The
...
An experiment bears this out, and is fair to the methods (Figure~\ref{fig:ts}). On a
nonstationary series with known $\sigma_t$, fixed split conformal collapses far below
target once drift sets in, but the genuinely adaptive methods --- ACI, conformal PID
...
The same holds for an off-the-shelf probabilistic forecaster. Running the author's
\texttt{skaters} \citep{cotton_timemachines} --- a streaming package that emits a
predictive mean and standard deviation at each step --- as a baseline gives near-oracle
CRPS ($0.86$ against the oracle's $0.76$), because its predicted spread tracks the changing
volatility. Conformalizing it adds nothing to forecast quality: an adaptive (normalized)
conformal wrap re-levels its coverage to $1-\alpha$ at an essentially identical interval
score ($6.58$ versus $6.57$), while a naive split-conformal wrap on the nonstationary
series collapses (to $60\%$ coverage). This reproduces the experience the introduction
describes: the value was in the conditional density the forecaster already estimates, and
the conformal step supplies a coverage certificate, not sharpness.

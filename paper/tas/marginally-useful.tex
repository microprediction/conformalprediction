...
\begin{abstract}
\noindent
Conformal prediction supplies a finite-sample, distribution-free guarantee: a prediction
set that covers the outcome with at least a stated probability, averaged over inputs. The
guarantee is genuine, and it is routinely misread as evidence that the forecast underneath
is good. We separate the two. For a fixed location predictor whose residuals are
re-leveled by a single shared shape---the setting in which conformal prediction is used to
emit a predictive distribution---we show that the expected logarithmic-score shortfall
relative to the best possible forecast equals one information-theoretic quantity: the
mutual information between the residual and the input. Conformalization re-levels coverage
but cannot reduce this quantity, because it is a property of the predictor's shape class
rather than of calibration; no recalibration that ignores the input can touch it. Three
familiar cautions follow as consequences rather than slogans: marginal coverage is not
conditional, validity is insensitive to sharpness, and the guarantee requires
exchangeability. We give a two-coordinate diagnostic that keeps coverage and forecast
quality visibly distinct, and we delineate the tasks for which coverage is itself the
right objective, where conformal prediction is not merely sound but ideal.
\end{abstract}
...
conformal prediction is the principled, distribution-free way to ``add uncertainty.'' They
bolt it on. The number they actually report---a proper score such as the predictive
log-likelihood---does not improve. The natural diagnosis is a tuning failure. The real one
is a \emph{category error}: conformal prediction is a method for \emph{certifying the
coverage of a set}, and it is being asked to \emph{estimate a distribution}. These are
different kinds of object, graded by different instruments, and no amount of tuning turns
one into the other. None of this is a criticism of the method. It is a guide to where the
method belongs.

The mathematics is not in question. Given exchangeable data, a fitted predictor, and a
...
\citep{vovk2005algorithmic,lei2018distribution,angelopoulos2023gentle}. The trouble is
entirely in the reading of \eqref{eq:marginal}. The probability averages over $X_{n+1}$ as
well as $Y_{n+1}$: it is a statement about the \emph{average} future input, not about the
one in front of you. It is \emph{marginal} coverage, and as the foundational analyses
warned from the start, ``a good estimator must satisfy something more than marginal
coverage'' \citep{lei2014distribution}.

\paragraph{Contribution.}
This paper makes that warning exact. Our one new result (Section~\ref{sec:gap}) is an
identity. Fix a location predictor and let conformal prediction re-level its residuals by a
single shared shape---the conformal predictive system of \citet{vovk2019nonparametric},
which is precisely the mode in which conformal prediction outputs a full predictive
distribution. Then the expected log-score regret of that distribution relative to the
conditional oracle is \emph{exactly} the mutual information $I(R;X)$ between the residual
$R$ and the input $X$ (Proposition~\ref{prop:gap}). We call this the
residual-information gap. It is the quantity conformalization cannot touch: re-leveling
that ignores $X$ can change the shape but not the dependence, so it leaves $I(R;X)$
invariant. Reducing the gap requires modeling the conditional residual---heteroscedastic
scale, quantiles, or shape---which is exactly the activity the conformal step does not
perform. The identity admits five equivalent readings (Section~\ref{sec:gap}): a
...
We are explicit about provenance. The impossibility of distribution-free conditional
coverage is due to \citet{lei2014distribution} and \citet{barber2021limits}; the re-leveling
object is the conformal predictive system of \citet{vovk2019nonparametric}; the
decomposition of a proper score into calibration and sharpness is standard
\citep{gneiting2007probabilistic,gneiting2007strictly}. The contribution is to combine these
into one exact identity along the score axis, to read it five ways, and to draw from it a
two-coordinate diagnostic (Section~\ref{sec:plane}) on which conformalization moves a report
horizontally---toward exact coverage---and never vertically, toward a better score. Our
quarrel is with a conflation, not with conformal prediction: when coverage is itself the
deliverable (Section~\ref{sec:steel}), the method is not merely sound but often ideal.

\paragraph{Structure.}
Section~\ref{sec:related} places the result in the literature. Section~\ref{sec:setup}
fixes notation. Section~\ref{sec:marginal} restates, with attribution, three known limits of
the marginal guarantee. Section~\ref{sec:orthogonality} shows coverage and the log-score are
orthogonal for a residual score; Section~\ref{sec:gap} proves the residual-information gap;
Section~\ref{sec:plane} draws the coverage--score plane. Section~\ref{sec:exch} treats
...
\paragraph{The conflation we address.}
Applied and survey writing routinely calls the marginal-coverage guarantee
...
separating \emph{validity} from \emph{efficiency}; our quarrel is with the conflation, not
with the method. A recent critique strand makes related points independently: that valid
sets can be uninformative \citep{recht2024cover}, that interval length can be gamed while
marginal coverage is held fixed \citep{min2026coveragelength}, and that the guarantee is
misread in applied domains \citep{pitfalls2025medical}. We add an exact identity for the
log-score regret, which these treat operationally rather than in closed form.

\paragraph{Foundations and the marginal guarantee.}
...
into a full predictive distribution by re-leveling the marginal residual law; it is the
object our regret identity grades.

\paragraph{The limits of marginal coverage.}
That marginal coverage is not conditional, and that distribution-free conditional coverage
is unattainable at finite length, is due to \citet{lei2014distribution} and sharpened by
\citet{barber2021limits}; \citet{vovk2012conditional} studies conditional validity for
inductive conformal predictors. Approaches that approach the conditional goalpost under
extra structure include \citet{gibbs2025conditional} and \citet{laplante2026pivotal}, and
diagnostics that quantify the conditional gap include \citet{braun2025conditional}. These
condition on $X$ or assume structure; none closes the gap distribution-free, consistent with
the impossibility results.

\paragraph{Proper scores, calibration, and recalibration.}
...
adjust a forecast's probabilities; we use the log-score throughout and relate what
$X$-blind recalibration can and cannot do to the gap.

\paragraph{Information-theoretic and conditional conformal.}
Closest to us is \citet{correia2024information}, who connect conformal set size to
information-theoretic uncertainty via bounds on expected set size in terms of the conditional
entropy $H(Y\mid X)$. That work grades the \emph{coverage/efficiency} axis; we grade the
\emph{score} axis, and our identity is for the log-score regret of the predictive
distribution rather than for set size. The two are complementary, and we make the connection
precise in Section~\ref{sec:plane}. Conditional and shape-adaptive conformal constructions
that leave the single-shape class---conformalized quantile regression
...
\citep{stutz2022learning}---are discussed in Section~\ref{sec:scope}, where we show they
confirm the paper's mechanism: they buy sharpness by conditioning on $X$ or optimizing a
proper objective, which is exactly what the gap says is required.
